...
export function ReviewModal({ isOpen, onClose, onSubmit, sections, answers }: any) {
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-2xl h-[80vh] flex flex-col p-0 overflow-hidden">
                ...
                <div className="space-y-4">
                  {s.questions.map((q: any) => (
                    <div key={q.id} className="flex justify-between items-start gap-4 py-3 border-b border-border/50">
                      ...
                      <span className="font-black text-primary px-3 py-1 bg-primary-soft rounded-md">
                        {answers[q.id] || "—"}
                      </span>
                    </div>
                  ))}
        ...
        <DialogFooter className="p-6 border-t shrink-0 gap-3 sm:gap-0 bg-surface">
          <Button variant="ghost" onClick={onClose} className="font-bold">Go Back</Button>
          <Button onClick={onSubmit} className="bg-primary hover:bg-primary-hover font-bold px-8">
            Confirm & Submit
          </Button>
        </DialogFooter>
      </DialogContent>
