...
import { updateAdminUserSchema } from "@/features/users/schemas/update-user";
import { createAdminUser } from "@/features/users/services/create-admin-user";
import { updateAdminUser } from "@/features/users/services/update-admin-user";
...
export async function updateAdminUserAction(formData: FormData): Promise<ActionResult> {
  ...
  const result = await updateAdminUser(parsed.data);
