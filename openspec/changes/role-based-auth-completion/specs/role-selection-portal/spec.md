...
- **Given** the portal page is loaded
- **When** the user views the FACULTY, STUDENT, ALUMNI, or INDUSTRY_PARTNER role cards
...
Selecting a self-service role card (FACULTY, STUDENT, ALUMNI, INDUSTRY_PARTNER) SHALL trigger Google OAuth sign-in with the selected role passed as an `intent` parameter. The OAuth redirect URL MUST include the role intent so the auth callback can determine domain requirements and post-login destination.
